...
if(glcolor.r==glcolor.g&&glcolor.g==glcolor.b)diffuse.rgb*=mix(mix(1.,mix(shadowDepth,1.,saturate(dot(sunPos,glnormal))),mainShadow),1.,sideShadow);
if(isPlant>.5)diffuse.rgb*=mix(1.,mix(shadowDepth,1.,saturate(dot(sunPos,glnormal))),mainShadow);
#endif
...
diffuse.rgb+=mix(vec3(0.),mix(vec3(.02),vec3(.3),day)*clamp(lmcoord.y,.2,.3),pow(saturate(1.-abs(wave)),1.6));
}
#endif

#ifdef GLOWING_ORES
vec3 t=texture2D(texture,texcoord).rgb;
if(isOre>=.5)if(t.r!=t.b||t.r!=t.g||((t.r+t.g+t.b)/3.)==1.)diffuse.rgb=tonemapFilmic(t*t);
#endif
